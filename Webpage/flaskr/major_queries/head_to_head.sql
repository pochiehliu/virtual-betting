WITH team_last_season AS(
    ...
    AND (t_id_home = {tid} OR t_id_away = {tid})
    ORDER BY game_time
),
...
SELECT ROUND(CAST(CAST(COUNT(
  CASE
    WHEN ((H = {tid} AND HA = TRUE) OR (A = {tid} AND HA = FALSE))
    THEN 1
  END
) AS FLOAT) * 100 / CAST(COUNT(*) AS FLOAT) AS NUMERIC), 3) AS R
FROM team_win_lose;
