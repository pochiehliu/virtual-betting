...
SELECT ROUND(CAST(CAST(SUM(field_goals_made) AS FLOAT) * 100 / CAST(SUM(field_goal_attempts) AS FLOAT) AS NUMERIC), 3) AS FGP
...
SELECT ROUND(CAST(CAST(SUM(three_pointers_made) AS FLOAT) * 100 / CAST(SUM(three_point_attempts) AS FLOAT) AS NUMERIC), 3) AS TPP
FROM game, player_game_stats
WHERE (game.t_id_home = {tid} OR game.t_id_away = {tid}) AND
       game.g_id = player_game_stats.g_id AND
       player_game_stats.t_id = {tid};

SELECT ROUND(CAST((SUM(offensive_rebounds) + SUM(defensive_rebounds))/ AVG(C) AS NUMERIC), 3) AS R_per_game
FROM game, player_game_stats,
...
SELECT ROUND(CAST(SUM(turnovers) / AVG(C) AS NUMERIC), 3) AS T_per_game
FROM game, player_game_stats,
     (SELECT COUNT(*) AS C FROM game WHERE game.t_id_home = {tid} OR game.t_id_away = {tid}) AS Total
WHERE (game.t_id_home = {tid} OR game.t_id_away = {tid}) AND
       game.g_id = player_game_stats.g_id AND
